...
const EditMessagePopover = React.createClass({
	...
	submit(event) {
		...
		const { messageKey, messageComponent } = this.state;
		...
		$.ajax({
			...
			success: (data) => {
				Messages._messages[messageKey] = text.s;
				
				if (messageComponent) {
					messageComponent.updateText(text.s);
				}
		...
		return (<div ref='main' className="fpdi-message-edit-popup"
			...
			<form onSubmit={this.submit}>
				<div className="form-group">
					<input className="form-control" defaultValue={this.state.text} ref="text" maxLength={55}/>
				</div>
...
const EditableMessage = React.createClass({
	...
	onContextMenu(event) {
		...
		if (this.context.roles.ADMIN ||
			...
			event.preventDefault();
			EditCt.show(this.props.messageKey, this.state.text,
