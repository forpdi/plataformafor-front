...
import FavoriteTree from "forpdi/src/forpdi/planning/widget/plan/FavoriteTree.jsx";
import TreeView from "forpdi/src/forpdi/core/widget/treeview/TreeView.jsx";
import LevelSearch from "forpdi/src/forpdi/planning/widget/search/LevelSearch.jsx";
import SearchResult from "forpdi/src/forpdi/planning/widget/search/SearchResult.jsx";
...
import { hasAddAndDeletetructureItemPermission, isResponsibleForSomeParent } from 'forpdi/src/forpdi/helpers/SthructureLevelsHelper.js';
...
export default React.createClass({
	...
	componentDidMount(){
        ...
        StructureStore.on("levelInstanceFind", (data, opts) =>{
						const { data: models, parents } = data;
            var children = [];
            if(models && models.length > 0){
	            ...
	            children.push({
									hidden: !this.hasPermission(parents, models.length == undefined ? models.goal : models[0].level.goal),
	                label: label,
		...
		DocumentStore.on("sectionAttributesSaved", (model) => {
			...
			if(nodeActive.length > 0) {
				var idx = nodeActive[1].innerText.split(" - ")[0];
				if(model.data.name.length > 50) {
					nodeActive[1].innerText = (idx +" - "+ model.data.name.substring(0,50) + "...");
				} else {
					nodeActive[1].innerText = idx +" - "+ model.data.name;
				}
			}
		}, me);
	...
	hasPermission(parents, isGoal) {
		const { roles, permissions } = this.context;

		return hasAddAndDeletetructureItemPermission(roles, permissions, isResponsibleForSomeParent(parents), isGoal);
	},
			...
			if (empty) {
				...
				Modal.exportDocument(
					...
					() => {
						...
						if(sections=='' || author.trim()=='' || title.trim()=='' || title.length > REPORT_TITLE_MAX_LENGTH){
							...
							if(title && (title.trim()=='' || title.length > REPORT_TITLE_MAX_LENGTH)) {
							...
							document.getElementById("documentAuthor").className = "";
							document.getElementById("documentTitle").className = "";
							var url = DocumentStore.url + "/exportdocument" + "?title=" + encodeURIComponent(title) + "&author=" + encodeURIComponent(author) + "&lista=" + lista;
							url = url.replace(" ", "+");
							Modal.hide();
	...
	exportCSV(evt) {
		...
		if (!exportingCSV) {
			const oneMinuteInMili = 60000;
			
			evt.preventDefault();
			this.setState({ exportingCSV: true });
			window.location.href = "forpdi/plan/export/"+this.props.plan.get("id");
			setTimeout(() => this.setState({ exportingCSV: false }), oneMinuteInMili);
		}
	},
	...
	render() {
		return (
		<div className="fpdi-tabs marginLeft20">
			<ul className="fpdi-tabs-nav marginLeft0" style={{justifyContent: 'space-between'}} role="tablist">
			...
			<div className="fpdi-tabs-content fpdi-plan-tree marginLeft0 plan-search-border">
				...
				:
					<div>
						...
						<a
							...
							style={this.state.exportingCSV ? { cursor: 'wait' } : {}}
							disabled={this.state.exportingCSV}
				...
				:
				<div className="fpdi-tabs-content fpdi-plan-tree marginLeft0">
