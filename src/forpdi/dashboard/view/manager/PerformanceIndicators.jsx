...
export default React.createClass({
	...
	componentDidMount() {
		...
		StructureStore.dispatch({
			action: StructureStore.ACTION_GET_OBJECTIVES,
			data: {
				macroId: (this.props.plan && this.props.plan != -1) ? (this.props.plan.get("id")) : (null),
				planId: (this.props.subPlan && this.props.subPlan != -1) ? (this.props.subPlan.id) : (null)
			}
		});
	},
		...
		for (var cont = 1; cont < goal.attributeList.length; cont++) {
			...
			if (goal.attributeInstanceList[index]) {
				if (goal.attributeList[cont].expectedField) {
					...
					fRec = goal.attributeInstanceList[index].valueAsNumber || "0";
				}
			}
		}
		var graphItem = ["", 0, '', 0];
